    ...
    ? cardIds
        ...
        .map((card, index) => {
          ...
          return {
            ...
            index,
            preview,
            templateName: templatesById[card.templateId]?.name ?? t("unknownTemplate"),
          }
        })
    : []
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10 md:flex-row">
      <aside className="flex max-h-[70vh] flex-col gap-2 overflow-y-auto border border-border/60 bg-card/40 p-4 md:w-64 md:rounded-xl">
        ...
        {isLoading ? (
          ...
          <p className="py-4 text-sm text-muted-foreground">{t("emptyDecks")}</p>
        ) : (
        ...
        {isLoading ? (
          ...
          <>
            <header className="space-y-2">
              <h1 className="text-3xl font-semibold tracking-tight">{selectedDeck.name}</h1>
            ...
            {cardsForDeck.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {cardsForDeck.map(({ card, index, preview, templateName }) => (
                  <UiCard key={card.id} className="flex h-full flex-col justify-between">
                    <UiCardHeader>
                      <UiCardTitle className="text-base font-semibold">
                        {preview || t("noPreview")}
                      </UiCardTitle>
                      <UiCardDescription className="text-xs">{templateName}</UiCardDescription>
                    </UiCardHeader>
                    <UiCardContent className="flex items-center justify-between gap-2 pt-0">
                      ...
                      <Button asChild size="sm" variant="outline">
                        <Link href={`/deck?deckId=${card.primaryDeckId}&cardIndex=${index}`}>
                          {t("openCard")}
                        </Link>
                      </Button>
                    </UiCardContent>
